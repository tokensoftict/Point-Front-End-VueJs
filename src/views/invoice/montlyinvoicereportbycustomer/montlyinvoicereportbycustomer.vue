<template src="./montlyinvoicereportbycustomer.html"></template>
...
export default
{
  ...
  methods: {
    ...
    filterReport()
    {
      ...
      this.invoiceService.custom({'filter':{'between':[this.from,this.to],"customer_id":this.$refs.customer_id.getValue()}})
          .then((response)=> {
            this.$refs.go.toggleProcessing();
  ...
  mounted() {
    this.$refs.go.toggleProcessing();
    this.invoiceService.$customer.get().then((customers)=>{
      this.customers = customers.data.data;
      this.invoiceService.custom({'filter':{'between':[this.from,this.to],"customer_id":1}})
          .then((response)=> {
            this.$refs.go.toggleProcessing();
            this.invoiceList = response.data.data.data
            this.columns = response.data.data.columns
