<template src="./montlyinvoicereportbyuser.html"></template>
...
export default
{
  ...
  methods: {
    ...
    filterReport()
    {
      ...
      this.invoiceService.custom({'filter':{'between':[this.from,this.to],"customer_id":this.$refs.user_id.getValue()}})
          .then((response)=> {
            this.$refs.go.toggleProcessing();
  ...
  mounted() {
    ...
    this.invoiceService.$accessControl.getUsers().then((users)=>{
      ...
      this.invoiceService.custom({'filter':{'between':[this.from,this.to],"created_by":1}})
          .then((response)=> {
            this.$refs.go.toggleProcessing();
            this.invoiceList = response.data.data.data
            this.columns = response.data.data.columns
