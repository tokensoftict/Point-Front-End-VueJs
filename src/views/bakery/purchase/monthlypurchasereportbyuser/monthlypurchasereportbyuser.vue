<template src="./monthlypurchasereportbyuser.html"></template>
...
export default
{
  ...
  methods: {
    ...
    filterReport()
    {
      ...
      this.purchaseService.custom({'filter':{'between':[this.from,this.to],"created_by":this.user_id}})
          .then((response)=> {
            this.$refs.go.toggleProcessing();
  ...
  mounted() {
    this.$refs.go.toggleProcessing();
    this.purchaseService.$accessControl.getUsers().then((sup)=>{
      this.users = sup.data.data
      this.purchaseService.custom({'filter':{'between':[this.from,this.to],"created_by":1}})
          .then((response)=> {
            this.$refs.go.toggleProcessing();
            this.purchaseList = response.data.data.data
            this.columns = response.data.data.columns
