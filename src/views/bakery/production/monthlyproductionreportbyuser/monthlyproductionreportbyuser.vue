<template src="./monthlyproductionreportbyuser.html"></template>
...
export default
{
  ...
  methods: {
    ...
    filterReport()
    {
      ...
      this.productionService.custom({'filter':{'between':[this.from,this.to],"user_id":this.$refs.user_id.getValue()}})
          .then((response)=> {
            this.$refs.go.toggleProcessing();
  ...
  mounted() {
    this.$refs.go.toggleProcessing();
    this.productionService.$accessControl.getUsers().then((users)=>{
      this.users = users.data.data;
      this.productionService.custom({'filter':{'between':[this.from,this.to],"user_id":1}})
          .then((response)=> {
            this.$refs.go.toggleProcessing();
            this.productionList = response.data.data.data
            this.columns = response.data.data.columns
